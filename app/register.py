...
def register():
    ...
    if username == "" or password == "" or confirm_password == "":
        ...
        return render_template("register.html", error_msg=error_msg, username=username, password=password, confirm_password=confirm_password)

    if password != confirm_password:
        error_msg = "Error: Passwords are not equal"
        return render_template("register.html", error_msg=error_msg, username=username, password="",confirm_password="")

    # input string validation
    for c in username:
        if c not in username_char:
            error_msg = "Error: Username must not contain character: " + c
            return render_template("register.html", error_msg=error_msg, username=username, password="")
    for c in password:
    ...
    if cursor.fetchone()[0]:
        ...
        return render_template("login.html", error_msg=error_msg, username="", password="", confirm_password="")
